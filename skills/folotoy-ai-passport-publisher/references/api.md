...
Submission endpoints accept multipart fields `title_zh`, `description_zh`, optional `title_en`, `description_en`, and `github_url`, plus `cover` and `firmware`. The cover must be an exact portrait 3:4 image. The historical optional `github_url` field name accepts GitHub, Gitee, GitLab, Codeberg, or another public HTTPS Git repository page. Server validation remains authoritative.
